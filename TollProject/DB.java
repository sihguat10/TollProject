...
/home/wcginte7/tollintl
     ...
     *  the Record object. 
     */
    public void exportRecords( ArrayList<Integer> numbers, boolean isPDF, boolean isJPG){
        PrintWriter pw; 
        BufferedReader br;
        ...
        pw.println("<html>");
        pw.println("<head>");
        pw.println("<title> ResumeToll </title>");
        ...
        for(int num: numbers){
            ...
            DBExport.makeResume(recList.get(num-1), br, pw);
        }
        pw.println("</body>");
        pw.println("</html>");
        pw.close();
        ...
        if(isJPG){
            Process p;
            try {
                String[] cmd = { "sh", "~/TollProject/createjpg.sh"};
                p = Runtime.getRuntime().exec(cmd); 
                p.waitFor(); 
                BufferedReader reader=new BufferedReader(new InputStreamReader(
                p.getInputStream())); 
